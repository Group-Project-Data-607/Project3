...
We began by searching various websites for suitable data before finally settling on Kaggle at (https://www.kaggle.com/code/lukebarousse/data-analyst-skill-analysis/input?select=gsearch_jobs.csv ). The file was then uploaded into our Github Project 3 repository as "Data_Science_job.csv."
...
The dataset that we chose contained many of the elements that we determined were necessary for this analysis - a variety of job listings from various companies from all over country, each listing desired skills for the ideal candidate.  While ideal from a data entry standpoint, the major challenge that we encountered  was that the data was far from tidy. There were multiple skills listed in one variable, vague job titles that did not match the job description, and way too many variables for the task at hand.  Thus let the tidying begin. 

###Titling the data
This data included job descriptions that were vague and made it difficult to discern(?) the true nature of the position.  Therefore, we took the liberty of creating job titles based on their respective descriptions, and group the jobs as "Data Analyst", "Data Engineer", "Data Scientist".
...
# remove special characters across dataframe
...
There are a number of technological and data visualization skills that employers consider essential for data scientist candidates.  Also know as "hardskills", these include competence in the latest software and statistical analysis tools. Here, our next task was filter and group like software skills in preparation for analysis. 
...
ggplot(hardskills_engineer,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="chartreuse1") + xlab('') + ylab('Frequency') + labs(title='Data Engineering Required Hard Skills') + coord_flip()
```
...
ggplot(softskills_engineer,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="orange") + xlab('') + ylab('Frequency') + labs(title='Data Engineering Required Soft Skills') + coord_flip()
```
...
In conclusion, based on the data shown, the top 5 hard skills are 

```{r}
# Create and display the first plot
plot1 <- ggplot(hardskills_science,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="red") + xlab('') + ylab('Frequency') + labs(title='Data Science') + coord_flip()

# Create and display the second plot
plot2 <- ggplot(hardskills_engineer,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="chartreuse1") + xlab('') + ylab('Frequency') + labs(title='Data Engineering') + coord_flip()

# Create and display the third plot
plot3<- ggplot(hardskills_analyst,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="purple") + xlab('') + ylab('Frequency') + labs(title='Data Analyst') + coord_flip()
...
```{r}
# Create and display the first plot
plot1 <- ggplot(softskills_science,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="blue") + xlab('') + ylab('Frequency') + labs(title='Data Science') + coord_flip()

# Create and display the second plot
plot2 <- ggplot(softskills_engineer,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="yellow") + xlab('') + ylab('Frequency') + labs(title='Data Engineering') + coord_flip()

# Create and display the third plot
plot3<- ggplot(softskills_analyst,aes(x=reorder(skill, freq), y=freq)) + geom_bar(stat='identity',fill="brown") + xlab('') + ylab('Frequency') + labs(title='Data Analyst') + coord_flip()
